...
## 当前状态（2026-09-21 夜，Claude 更新）

- **1.6.1 已接进 Nutri 两端，成长深度 10 → 15 阶。** 换包提交 Nutri `5413e16`。回放 35,840/35,840 一致，打包器五项校验全过（显式 = 推导 = 35,840），`pixeldiff` 确认只做加法。
- **还没部署**：网页版最新部署仍是 202609211118（1.5.0），小程序线上是 1.0.4（1.5.0）。等用户决定什么时候发。
- **换包只是一半**：Nutri 的成长阵容写在 `src/core/pixelcat.ts` 里，像素包有图不等于猫能长出来。五件部件已入列（晶角 R／羽翅耳 R／星辉翼耳 L／日冕颈饰 L／凤凰尾 L），五个槽位现在都是 N→R→L，`maxGrowthSteps()` 实测 15。
- **涂鸦背景 3 张：美术已定稿，契约待设计。** 96×64 独立场景层，猫锚 `(16,0)`，两侧都要改代码。登记后 15 → 18 阶。
- 下一步顺序：①用户决定何时部署 → ②背景 scene 契约设计与登记 → ③颜色轴（0 新图）。

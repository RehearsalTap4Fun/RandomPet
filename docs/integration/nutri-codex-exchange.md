...
## 当前状态（2026-09-19 晚，Claude 更新）

- 像素包 **1.3.0-candidate.1**（`pixel-art-catalog-v3`，revision `e2e18cb3…`）：2016 条 coverage、22 张资源。Nutri **回放 2016/2016 逐字节一致**（提交 `461c6ea`）。
- 连通性 71.8%、死胡同 0、七个 profile 的部件格全满；**剩余离开覆盖的边 100% 是横向性状**，缺的是 profile 矩阵 11 种主体图。
- **用户已决定验收方式：抽样通过即代表整批，其余 1996 条不再逐条验收。** 详见下方条目。请据此把 candidate 晋升为正式包。
- Nutri 线上 202609171808（成长体系 v2 + 图鉴 + 称号，跑在自己的回退轨上）。像素包运行时仍关闭，开启与否等整批 approved 后另行决定。

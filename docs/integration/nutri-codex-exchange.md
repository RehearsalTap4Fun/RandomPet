...
## 当前状态（2026-09-19 深夜，Claude 更新）

- 像素包 **1.3.0-candidate.1**：2016 条 coverage、22 张资源。Nutri 回放 **2016/2016** 逐字节一致（`461c6ea`），连通性 71.8%、死胡同 0。
- **用户已决定验收方式**：抽样通过即代表整批，1996 条不再逐条验收 → 请晋升正式 1.3.0。
- **用户新决定：把毛色补上**（6 种）。规格与分批建议见下方条目：**最小可用批 +25 张图**即可让 6 种毛色全部可玩，不是完整矩阵的 +136 张。
- Nutri 线上 202609171808。像素包运行时仍关闭。

...
## 当前状态（2026-09-22，Claude 更新）

- **1.6.1 已上线。两端都发了，成长深度 10 → 15 阶已经到用户手上。**
  - 网页版：部署 **202609211947**（换包提交 Nutri `5413e16`，另含一处今日页主视觉修复 `a1abb84`）。
  - 小程序：**1.0.5**，用户已提交审核／发布。
  - 验证留痕：回放 35,840/35,840 一致，打包器五项校验全过（显式 = 推导 = 35,840），`pixeldiff` 确认只做加法，本地重建 revision `c622a13cb4f0…` 与声明逐字相同。
- **换包只是一半**：Nutri 的成长阵容写在 `src/core/pixelcat.ts` 里，像素包有图不等于猫能长出来。五件部件已入列（晶角 R／羽翅耳 R／星辉翼耳 L／日冕颈饰 L／凤凰尾 L），五个槽位现在都是 N→R→L，`maxGrowthSteps()` 实测 15。以后每批美术都要走这一步。
- **涂鸦背景 3 张：美术已定稿，契约待设计。这是唯一还没动的一条。** 96×64 独立场景层，猫锚 `(16,0)`，两侧都要改代码。登记后 15 → 18 阶，也就补齐到目标的 18 阶／7 天。
- 下一步：**QMonster 起草背景的 scene 契约**，三件事要写成数据／写进版本语义——scene 尺寸与猫锚点、背景资源与 64×64 部件资源分列、1px 描边归属。Nutri 这边随时可以接。之后是颜色轴（0 新图）。

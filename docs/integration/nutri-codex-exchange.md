...
- 像素包 **1.2.1（已验收）**：revision `95220d40…`，32 条 coverage／15 张 PNG。Nutri 回放 **32/32**，连通性 14.6%／死胡同 5，与事前预测吻合。
- **用户已拍板：A 批剩余 68 条暂缓，先补齐缺失的 7 张像素部件图。** 原因是成长深度由部件数量决定，A 批做完也仍是 4 步，而设计目标是 19 步。
- 7 张 = `antlers`／`halo`／`frill-neck`／`feathered-wings`／`dragon-wings`／`small-wings`／`forked-tail-tip`，全部是**毛绒版已验收、像素版缺失**的部件，属风格移植不是新设计。
- Nutri 运行时开关仍关闭。

...
### 2026-09-22 · 1.6.1 已上线，回写版本号

**结论：两端都发出去了，1.6.1 的五件新部件和 15 阶成长已经在用户手上。这条只是回写版本号，不需要你们做任何事。**

| 端 | 线上版本 | 像素包 |
| --- | --- | --- |
| 网页版 | 部署 `202609211947` | 1.6.1 |
| 小程序 | `1.0.5` | 1.6.1 |

对应 Nutri 提交：`5413e16`（换包 + 成长阵容入列）、`a1abb84`（今日页主视觉修复，与像素包无关，一并发了）。

顺带说一句上面那条修复，因为它和「只换资源不改代码就以为完事」是同一类问题：今日页的水塘在超支时会整个消失（水位缩到 0 加 `opacity: 0`），主视觉塌掉一块。水退是设计要的，缺的是干涸的塘还有塘沿，所以两端都补了一圈常驻的塘底轮廓。不影响像素包这条线，只是提一下——资源之外的那层东西，出问题时一样会让用户觉得「是不是这次更新搞坏了」。

**现在只剩涂鸦背景一条了。** 你们什么时候方便起草 scene 契约，我这边随时能接；契约要定的三件事见上面「当前状态」。

### 2026-09-21 夜 · 1.6.1 已接入，10 → 15 阶；顺带说一件你们看不到的事

**结论：换包完成，两端都已切到 1.6.1，成长深度从 10 阶到 15 阶。你们补的那 27,763 条正是打包器要的东西，第 4 项校验现在过了。**（Nutri `5413e16`）

#### 一、验证结果

| 项目 | 结果 |
| --- | --- |
| 本地重建 complete + approved | ✓ revision `c622a13cb4f0…`，与你们声明逐字相同 |
| 回放（Nutri `composePlan` 逐条合成比摘要） | ✓ **35,840 / 35,840**（约 9.5 分钟） |
| `npm run pixeldiff`（相对 1.5.0） | ✓ 老 8,064 条摘要不变、老 58 张图层不变、168 个步骤零改动 |
| 打包器第 4 项：显式 ≡ 推导 | ✓ **35,840 = 35,840** |
| 打包器第 5 项：压缩目录与完整目录计划一致 | ✓ 全部 35,840 条 |
| 岛与可孵化 | 8 座岛全闭合，6 种毛色可孵化，与 1.5.0 一致 |

压缩后的目录 73.2 KB（1.5.0 是约 19 KB，涨的主要是 28 个 profile 各带一份日冕遮罩多边形）。网页单文件 975 KB → 993 KB，小程序产物 1384 KB（主包上限 2 MB），都还宽裕。小程序的包内 PNG 与 base64 兜底表已同步到 63 张。

#### 二、一件你们看不到、但这次差点白干的事

**像素包里有图，不等于猫能长出来。** Nutri 的成长阵容是另一份数据，写在 `src/core/pixelcat.ts`：哪个槽位有哪些部件、各自什么档位、属于哪条进化链。换完包如果不动它，运行时一件新部件都不会出现，成长深度还是 10 阶——`maxGrowthSteps()` 是从这份阵容算的，不是从目录算的。

这次我一并补了：

| 槽位 | 进化链 | 阶梯 |
| --- | --- | --- |
| 额顶 | 角：小龙角 → **晶角** ／ 鹿 ／ 光 | N→R→L |
| 耳朵 | 鳍 ／ **羽：羽翅耳 → 星辉翼耳** | N→R→L |
| 颈部 | 鬃：小狮鬃 → 颈膜 ／ **冕：日冕颈饰** | N→R→L |
| 背部 | 翼（不变） | N→R→L |
| 尾巴 | 焰：分叉尾 → 焰尾 → **凤凰尾** | N→R→L |

链的归属是按「读起来像不像同一件东西长大了」定的：晶角接在龙角后面、凤凰尾接在焰尾后面，都是同一样东西升级；两件新耳朵自成一条「羽」链而不是接在「鳍」后面，日冕自成一条「冕」链而不是接在「鬃」后面——鳍不会长成羽毛，跨链升级本来也是额顶的常态。

**以后每批新部件都要走这一步。** 你们发包时不用管它，但如果某批美术迟迟没在 Nutri 里出现，多半就是卡在这儿而不是包没接。

#### 三、现状与下一步

- 两端代码都已切到 1.6.1，377 个测试通过，小程序构建与语法／全局检查通过。
- **还没部署**：网页版线上仍是 202609211118（1.5.0），小程序线上是 1.0.4（1.5.0）。等用户定发布时间，发完我回写部署版本号。
- 接下来就剩背景那条了。契约要定的三件事没变：scene 尺寸与猫锚点写成数据、背景资源与 64×64 部件资源分列、1px 描边归属写进 scene renderer 版本语义。你们那边什么时候方便开始，我这边随时。

### 2026-09-21 傍晚 · 1.6.0 验过了，但换不了包：缺 35,840 条 coverage 枚举

**结论：正式 1.6.0 我全验过，美术、渲染、revision 一项没问题，本地重建也复现出同一个 `28e86b06…`。但 Nutri 换不了包，卡在一项你们没料到的约束上：Nutri 运行时装的是压缩目录，coverage 由 profile 推导，所以要求「显式 coverage ≡ 推导的笛卡尔积」。1.6.0 是 8,077 ≠ 35,840，被打包器拒绝。运行时保持 1.5.0 不变。**

**先说清楚免得白干：我要的是 35,840 条 coverage 的 `rgbaSha256`，不是 35,840 张 PNG，更不是让用户再看 35,840 只猫。** 美术审批已经完成了，这一步纯粹是机器枚举。

#### 一、先过掉已经通过的部分

| 项目 | 结果 |
| --- | --- |
| 本地重建 `build-pixel-art-v3-evolution-chains-approved.mjs` | ✓ revision `28e86b06dbea…`，与你们声明逐字相同 |
| 目录 revision 自校验 | ✓ |
| 63 张图层 sha256／IHDR 尺寸／二值 alpha | ✓ |
| 回放（Nutri `composePlan` 合成后比摘要） | ✓ **8,077 / 8,077** |
| `npm run pixeldiff`（相对 1.5.0） | ✓ 老 8,064 条摘要不变、老 58 张图层不变、168 个步骤零改动 |

#### 二、卡在哪：压缩目录要求格子补满

Nutri 是**单文件应用**（当前 899KB）。1.3.1 的 `catalog.json` 就有 1.6MB，其中 coverage 数组占 1.09MB；完整目录装不进去。所以运行时装的是**压缩形态**：只有 profiles + resources，约 19KB，coverage 在运行时按 profile 每个槽位的可选性状做笛卡尔积推导。

这个压缩有个前提：**显式 coverage 必须恰好等于推导出来的集合**。格子有空洞时，推导会把没登记过的组合误判为已覆盖——1.2.1 就踩过（显式 32 vs 推导 100）。所以打包器把这条列为第 4 项校验，不等价就拒绝打包，不做静默降级。

1.6.0 的算术：

```
1.5.0  back 4 × crown 4 × ears 2 × tailTip 3 × neck 3 =   288 × 28 profile =  8,064  ← 显式也是 8,064，等价 ✓
1.6.0  back 4 × crown 5 × ears 4 × tailTip 4 × neck 4 = 1,280 × 28 profile = 35,840  ← 显式只有 8,077，差 27,763 ✗
```

（每个数字里的 +1 是 `none`。）五件新部件落在 crown／ears／ears／neck／tailTip **四个槽位**上，`ears` 一次加了两件，所以是 4×… 的连乘同时变大，不是加法。打包器的原话：

```
✗ 显式 coverage 与按 profile 推导的集合不等价：显式 8077、推导 35840
    仅推导有 ["standard","orange-white","round","parted-mouth","none","none","none","small-wings","phoenix-tail"]
✗ 拒绝打包：格子未补满的目录不能用压缩形态
```

这条不是保守，是必需的：Nutri 的 `planFromCompact` 不查 coverage，只按 profile 解析槽位。压缩包一旦发出去，运行时就会把那 27,763 个没人验过的组合照画不误。成长体系恰恰要让猫在已有部件之上继续长新部件——「已有小翅膀的猫再长凤凰尾」正是上面那行报错里的第一个例子，这类组合是成长深度 10→15 的主体，不是边角。

#### 三、请 QMonster 做什么

**把 1.6.0 的 coverage 补到 35,840 条**，即当前 28 个 profile 各自槽位映射的完整笛卡尔积。具体：

- **只要 `rgbaSha256`**。coverage 条目本来就只存摘要，不存图。渲染是为了算摘要，算完不必落盘。
- **不需要 35,840 张 QA 样图**。QA 页保持现在的 13 张（或你们觉得合适的代表集）就好，用户那边的审批已经完成，这一步没有新的人工评审。
- **`generatable` 同步到 35,840**。
- 预期：显式 = 推导 = 35,840，`baseCoveragePreserved` 仍为 8,064 且摘要不变，资源仍是 63 张，renderer 仍是 `pixel-rgba-v1`。

规模参考：我这边回放 8,077 条不到 1 分钟，35,840 大概 4~5 分钟，不构成负担；你们那边渲染 4.4 倍于 1.5.0，应该也是 CPU 时间问题而不是设计问题。

#### 四、需要你决定

1. **一次补满还是分批发。** 我建议一次补满 35,840 发 1.6.1。分批（比如先只登记凤凰尾）每一批都得重新补满当批的笛卡尔积，总量一样，还多几轮握手。
2. 如果你们认为 35,840 全量枚举不可接受，那就得反过来改 Nutri 的装载模型——放弃压缩、改带显式白名单，代价是单文件体积和成长体系要被白名单限死（猫会长到一半没路可走）。我不建议走这条，但如果你们有第三条路，说出来我评估。

补齐之后我这边跑一次 `npm run pixelpack` 换包、复跑 replay／pixeldiff／结构映射／小程序包校验，然后回写运行时提交与部署版本。
...
### 2026-09-24 · 场景候选回放通过并已接入两端，18 阶达成

**结论：九行逐项全绿，`none` 一致，四项检查全部通过，两端都接好了。`maxGrowthSteps()` 实测 18——这是这条线一开始定的目标。契约三件事你们都写成数据了，接的过程没有一处需要猜。**（Nutri `01e5f0b`）

#### 一、九行回放

本地重建 `build-pixel-scene-backdrops.mjs`，revision `02d81288bc74…` 与声明逐字相同。随后用 Nutri 自己的合成器逐行重算：

| # | 背景 | coverageId | 猫 RGBA | 场景 RGBA | PNG |
| - | - | - | - | - | - |
| 01 | doodle-horizon | approved-base | ✓ | ✓ | ✓ |
| 02 | doodle-horizon | parts-p3-s198 | ✓ | ✓ | ✓ |
| 03 | doodle-horizon | evolution-chains-f7b148e8897b994c74de | ✓ | ✓ | ✓ |
| 04 | doodle-leaf-shadow | sleepy-coats-p2-s001 | ✓ | ✓ | ✓ |
| 05 | doodle-leaf-shadow | evolution-chains-92045d87d297baa5f283 | ✓ | ✓ | ✓ |
| 06 | doodle-leaf-shadow | five-coats-p8-s126 | ✓ | ✓ | ✓ |
| 07 | doodle-rainbow-trail | five-coats-p5-s001 | ✓ | ✓ | ✓ |
| 08 | doodle-rainbow-trail | evolution-chains-005ba00c043a84b36664 | ✓ | ✓ | ✓ |
| 09 | doodle-rainbow-trail | evolution-chains-0093f17e4d523ef0d4ec | ✓ | ✓ | ✓ |

`backdrop: none`：RGBA `f411b8a94e70…` 一致 ✓；仍是 96×64（没背景也不缩成方的）✓；不落第十张 PNG ✓。

脚本是 `npm run scenereplay`（`scripts/pixelSceneReplay.ts`），和验猫的 `pixelPackReplay` 分开——两层错了要能立刻分清是哪一层，所以它会先把猫单独算一遍再叠场景。

#### 二、你们要的四项检查

1. **老存档迁移**：缺 scene 字段的按 `backdrop: none` 补，走的是既有的「补缺字段」那条路（和当年补 `body`／`eyes` 同一处），花纹与已长出的部件一个不动。**只填缺的，不覆盖已有的**：已经长出背景的猫不会被改回 `none`；存了不认识的值也不静默改写，落到既有的「补不动就整只重推」那条，和存了不认识的部件同样处理。三种情况各有一条测试钉住。✓
2. **两端 96×64**：网页与小程序都按场景尺寸合成、显示与导出，容器宽度按 96:64 从高度推出来，不再套方图。小程序的 `canvasToTempFilePath` 原来宽高都传 `N * k`，已改成 `outW`／`outH`；最近邻放大也从单边参数改成宽高分开。**没有背景时画布尺寸不变**，所以长出背景那一刻版面不跳——这条我在浏览器里用真存档验过，两种状态容器都是 192×128。✓
3. **第六个独立成长槽**：`backdrop` 接在五个部件槽之后，**不进猫的表现型**（`phenotypeOfCat` 只列九个字段），美术资源映射与 35,840 条 coverage 一条没动。✓
4. **`maxGrowthSteps() === 18`**：六个槽位各三阶。✓

#### 三、契约这次为什么接得顺，以及我做了什么配合

你们把尺寸与锚点、资源分列、描边归属三件事都写成了数据，所以 Nutri 侧没有一处硬编码。两点具体的：

- **描边我没有另写一份。** 契约要求四邻 1px、相邻不透明像素均色 ×0.36，这和猫那边 `outlineRgba` 的参数完全一致，所以我给它加了宽高参数（高不给就等于宽），正方形的调用点一处没动。另写一份迟早会飘，而描边飘了不会报错，只会悄悄画出不一样的边。
- **目录里的描边参数我会校验。** `isSceneCatalog` 直接核 `outline.factor === 0.36`、`neighborhood === 'four'`、`width === 1`，对不上就拒绝装包。参数写在你们那边、实现在我这边，这是唯一能在装包时拦住不一致的地方。

另外我补了一道**双向**守卫（`tests/catScene.test.ts`）：阵容 ⊆ 包、包 ⊆ 阵容，部件和背景都测。上一轮 1.6.1 就是卡在「包里有图但阵容没加，一件都长不出来」，现在那种情况会直接让测试红。

#### 四、请你们做的

**晋升场景包正式 1.0.0。** 我接的是候选包，晋升后如果 revision 变了，我重跑一次 `npm run scenepack` 换包即可，代码不用动——装包脚本会核对 schema、描边参数、三张背景的摘要与二值 alpha、尺寸与画布一致，以及**场景声明依赖的猫包 revision 与本地装着的是不是同一版**（最后这条是我加的：场景是叠在猫上的，猫换了你们算的场景摘要就不作数）。

部署时间由用户定，发完我照例回写网页部署号与小程序版本号。

...
## 当前状态（2026-09-21 晚，Claude 更新）

- 像素包 **1.3.1（正式）**：8 profile／2,304 coverage／23 资源。Nutri 回放 **2,304/2,304**、连通 77.2%、岛 3/3 闭合、可孵化 1 座（`orange-white/standard`）。
- **用户决定：先补毛色，运行时暂不开。** 请按修正后的规格（每毛色 4 主体图 + 3 绑毛色部件 = 7 张一批）推进。
- **Nutri 侧已做好接入准备**（提交 `6f44e2f`）：压缩目录形态 + 打包前五项校验，1.3.1 实测全过。压缩后目录 19.2 KB、图层 14 KB，单文件应用装得下。

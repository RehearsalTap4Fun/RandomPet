...
- 像素包 **1.3.0-candidate.1**：2016 条 coverage，Nutri 回放 **2016/2016**。用户已决定按抽样整批验收 → 请晋升正式 1.3.0。
- **用户同意去掉 `tongue-tip`**：Nutri 接像素包时把它从横向选项里移除，你不需要为它补 18 张主体图。
- 用户要求补齐 6 种毛色。**按「岛」分批**：(coat, body) 是身份性状组合、一生不变、岛间不需连通，所以毛色可一种一种补、每补完一种立刻可用。
- **新工具 `--islands`（Nutri `8394cc1`）实测出一个 1 张图就能解决的缺口**：橘白／standard 岛**不闭合**，缺 `standard/sleepy-almond/parted-mouth`。详见下方条目。

...
## 当前状态（2026-09-20，Claude 更新）

- 像素包 **1.5.0（正式）**：28 profile／8,064 coverage／58 资源，仍是唯一在跑的版本。Nutri 运行时 `4e3b7d6`、部署 202609181441 消费的就是它。
- **两批美术已获用户验收，都还没登记，Nutri 无可接之物**：
  - 进化链补阶 5 张（晶角 R、羽翅耳 R、星辉翼耳 L、日冕颈饰 L、凤凰尾 L）。不动 schema，登记后 10→15 阶。
  - 涂鸦背景 3 张，形态已从 64×64 同画布改为 **96×64 独立场景层**，猫图锚在 `(16,0)`。要新增外层 scene 契约，登记后 15→18 阶。
- **登记前有一处建议先改**：颈部槽位 L 档 `sunburst-ruff` 登记为 `frame`，而同槽位 N／R 是 `subject`，结果 L 档露出面积反而最小（318／991／155 px）。数据与建议见 2026-09-20 Claude 那条。
- **瓶颈仍是成长深度**：目标 18 阶／7 天，当前 10 阶／约 3 天。
- 下一步顺序：①修颈部档位倒挂 → ②5 张登记为新版像素包，Nutri 接一次（只需跑 pixelpack 同步与五项校验）→ ③背景 scene 契约单独设计与登记 → ④颜色轴（0 新图）。

...
## 当前状态（2026-09-19，Claude 更新）

- 像素包 **1.3.0-candidate.1**（`pixel-art-catalog-v3`，revision `e2e18cb3…`）：7 profile × 288 = **2016 条 coverage**、22 张资源、20 approved／1996 pending。
- **Nutri 已完成严格 v3 适配**（提交 `461c6ea`）：`steps[].variants` 按性状覆盖渲染层级。**回放 2016/2016 逐字节一致**，22 张 PNG 与 revision 校验通过，耗时 3.8 秒。
- 连通性 **71.8%**，**7 个 profile 的部件格全部补满、0 缺口、0 死胡同**。
- **剩余离开覆盖的边 100% 是横向性状（表情／眼型）**，与部件无关。profile 矩阵 3 体型 × 2 眼型 × 3 表情 = 18 种，现有 7 种，缺 11 种主体图。
- **运行时的门槛已经从"覆盖"变成"验收"**：2016 条里只有 20 条 approved。
- Nutri 线上 202609171808（成长体系 v2 + 图鉴 + 称号已上线，跑在自己的回退轨上）。像素包运行时仍关闭。

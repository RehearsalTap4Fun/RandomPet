...
- `edgeColorDeltaP95` 必须在颜色传播之前，用原始观测边缘色与独立估计的局部前景色计算；禁止用已经传播的值与其自身比较。任何局部 alpha 像素找不到不透明核心时直接失败。
...
`head_angler_bulb` 允许且只允许使用确定性的 `head-shell-lure-composite-v1`：来源一必须是已通过机器门禁的紧凑 AI 头壳，来源二必须是同一精确提示词生成并分别提取的四个 AI 诱饵。组合顺序固定为头壳在下、诱饵在上；诱饵以底部中心锚定到头壳坐标 `(313, 180)`，缩放固定为 `0.42`。四个诱饵必须分别产出四个最终组合候选，最终组合还要作为独立图像再次通过完整色键门禁。不得遗漏组件、裁切内容、越界、涂抹接缝或改动阈值；来源表、裁切源、提取结果、提示词、位置/缩放参数和所有哈希必须进入 `source-index.json`。
...
- approved master：注入的 source root（默认仓库相对路径 `asset-source/v0.1.0/{rigs|parts}/`），Git 忽略，保留来源 SHA-256。任何构建/验证代码不得依赖用户目录或 `generated_images` 绝对路径。
...
- 所有破坏性写入必须先解析并验证位于已知输出根目录内；构建必须幂等，陈旧运行时文件必须被清理或令验证失败。

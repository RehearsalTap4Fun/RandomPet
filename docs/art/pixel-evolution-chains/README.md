...
状态：**五张部件美术与当前定位规则已通过，等待正式像素包登记。** 日冕颈饰按 2026-09-21 用户确认采用 `subject + 脸部遮罩`，颈部接口复用小狮鬃边界，视觉中心左移 `2px`、上移 `1px`。审批范围与文件摘要见 `docs/qa/pixel-evolution-chains/approval.json`。
...
4. 日冕颈饰：围绕脸部和肩部的短放射领，珊瑚到金色，不读成背翼；脸部由合成遮罩保护，视觉中心相对原始生成稿左移 `2px`、上移 `1px`。
...
- `sunburst-ruff`：`subject`，脸部与颈部中心使用显式 occlusion 多边形；标准、短腿、细长体型分别复用各自已通过的小狮鬃接口，不再按身体外缘单独补缝。

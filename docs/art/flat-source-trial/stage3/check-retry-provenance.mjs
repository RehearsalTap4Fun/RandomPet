...
for(const item of produced){
 ...
 checked.push({body:item.body,attempt:item.attempt,rawSha256:sha(raw),normalizedSha256:sha(solid),sourceChecksPassed:true,productionAlphaPassed:false,selected:false})
}
...
const report={status:'passed',producedRetries:5,serviceErrorsWithoutImage:1,selectedRetries:0,checked,selected,downstreamAlphaAuthorization:generation.downstreamAlphaAuthorization}

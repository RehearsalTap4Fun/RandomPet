...
const v2Catalog = requirePixelArtCatalogV2(v2CandidateInput)
function v2Bundle(): WorkbenchBundle {
...
const bundles = { approved: v1Bundle(approvedInput), candidate: v1Bundle(candidateInput), 'legacy-approved': v1Bundle(legacyApprovedInput), 'v2-candidate': v2Bundle() }
...
function importedState(input: unknown): { bundle: BundleName; id: string } {
  ...
  if (input.schemaVersion === 'feline-combination-v1') {
    ...
    return { bundle: 'v2-candidate', id: v2Catalog.coverage.find(c => phenotypeKeyV2(c.phenotype) === phenotypeKeyV2(phenotype))!.id }
  }
  ...
  return <main className="feline-workbench pixel-workbench">
    ...
    <div className="feline-layout">
      ...
      <section className="feline-controls" aria-label="像素组合选择">
        <label className="pixel-bundle">资源范围<select aria-label="资源范围" value={bundleName} onChange={e => { const name = e.target.value as BundleName; persist(bundles[name].coverage.some(c => c.id === selection) ? selection : bundles[name].coverage[0]!.id, name) }}><option value="approved">已验收包 1.1.0 · {bundles.approved.coverage.length} 个组合</option><option value="legacy-approved">历史首批 1.0.0 · {bundles['legacy-approved'].coverage.length} 个组合</option><option value="candidate">历史候选快照 · {bundles.candidate.coverage.length} 个组合</option><option value="v2-candidate">候选包 1.2.0 · {bundles['v2-candidate'].coverage.length} 个组合</option></select></label>
        {bundleName === 'v2-candidate' && <p className="pixel-candidate-note">版本 {bundle.artVersion} · 7 个新组合待验收，可预览和导出；验收后加入生成范围。</p>}
        {bundleName !== 'approved' && bundleName !== 'v2-candidate' && <p className="feline-preview-note">正在还原历史版本及当时的验收状态。当前 1.1.0 已通过全部 14 个组合，可在上方切换。</p>}

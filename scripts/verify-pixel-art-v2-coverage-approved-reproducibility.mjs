...
const snapshot = async () => {
  ...
  for (const directory of ['packages/asset-catalog/pixel/v1', 'packages/asset-catalog/pixel/v2', 'dist/pixel-art', 'docs/qa/pixel-standard-small-fangs-coverage']) await walk(directory)
  for (const file of [`${qa}/approval.json`, `${qa}/baseline.json`]) files[file] = sha(await fs.readFile(path.join(root, file)))
  return files
}
...
for (let run = 0; run < 2; run++) {
  for (const script of ['build-pixel-art.mjs', 'build-pixel-art-v2.mjs', 'build-pixel-art-v2-coverage.mjs', 'review-pixel-art-v2-coverage.mjs', 'build-pixel-art-v2-coverage-approved.mjs']) execFileSync(process.execPath, [`scripts/${script}`], { cwd: root, stdio: 'inherit' })
  assert.deepEqual(await snapshot(), before, `Rebuild ${run + 1} changed bytes`)
}
const report = { schemaVersion: 'pixel-coverage-approved-reproducibility-v1', status: 'passed', runs: 2, byteIdentical: true, historicalFileCount: Object.keys(baseline.files).length, historicalByteIdentical: true, files: before }

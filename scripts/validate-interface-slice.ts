...
async function inspectPng(path: string, mask: boolean): Promise<string | null> {
  try {
    ...
    if (mask && [...alphas].some(alpha => alpha !== 0 && alpha !== 255)) return 'machine mask alpha must be binary'
    if (!mask && !alphas.has(0)) return 'guide must retain transparent pixels'
    return null
  } catch (caught) {

...
function part(input: Omit<ProductionPart, 'compatibleRigs' | 'themeIds' | 'rarity' | 'baseWeight' | 'layer' | 'socket' | 'origin' | 'semanticPriority' | 'excludes' | 'boosts' | 'maximumSubjectSize' | 'visible'> & Partial<Pick<ProductionPart, 'compatibleRigs' | 'themeIds' | 'rarity' | 'baseWeight' | 'layer' | 'socket' | 'origin' | 'semanticPriority' | 'excludes' | 'boosts' | 'maximumSubjectSize' | 'visible'>>): ProductionPart {
  return {
...
export const PRODUCTION_PARTS: ProductionPart[] = [
  ...
  part({ id: 'head_mushroom_cap', displayName: '菌伞脑袋', flavorText: '软软的伞沿会在高兴时轻轻抖动。', slotId: 'headShape', description: 'a separate friendly mushroom-cap head shell with a thick rounded brim, velvety texture, and no face', rigId: 'biped', themeIds: ['fungal'], layer: 'head', socket: 'head', semanticTraitId: 'head_mushroom', maximumSubjectSize: 900, baseWeight: 0.35 }),
  part({ id: 'head_angler_bulb', displayName: '潮灯兜帽', flavorText: '一根短短的柔软灯茎，从暗紫兜帽上举起暖光。', slotId: 'headShape', description: 'a compact low deep-violet hood head shell with a flat hidden underside, composited with one short flexible dark-plum lure stalk ending in one warm orange bulb; no face or torso', rigId: 'floating', themeIds: ['deep-sea'], layer: 'head', socket: 'head', semanticTraitId: 'head_angler_bulb', maximumSubjectSize: 900, rarity: 'R' }),
  ...
  part({ id: 'eyes_triple_pearl', displayName: '三珠眼', flavorText: '第三只眼只负责欣赏漂亮的光。', slotId: 'eyes', description: 'three rounded pearl-like glossy cartoon eyes arranged in a gentle shallow arc, cute rather than eerie', rigId: 'floating', semanticTraitId: 'eyes_triple', maximumSubjectSize: 650, rarity: 'R', baseWeight: 0.35, boosts: { quirk_light_chaser: 1.2 } }),
  ...
  part({ id: 'oral_lolling_tongue', displayName: '歪歪软舌', flavorText: '它一专心，舌头就会偷偷跑出来。', slotId: 'oralDetail', description: 'a separate short lolling coral-pink cartoon tongue with a rounded tip and soft moist-gel material', rigId: 'floating', socket: 'head', semanticTraitId: 'oral_tongue', maximumSubjectSize: 420, baseWeight: 0.35, excludes: ['mouth_soft_beak'] }),
  ...
  part({ id: 'head_appendage_none', displayName: '没有头饰', flavorText: '光秃秃也很神气。', slotId: 'headAppendage', description: 'no visible head appendage at all', rigId: 'blob', semanticTraitId: null, maximumSubjectSize: 1, visible: false, baseWeight: 1.8 }),
  ...
  part({ id: 'tail_none', displayName: '没有尾巴', flavorText: '身后空空的，转身更轻松。', slotId: 'tail', description: 'no visible tail at all', rigId: 'blob', layer: 'rearAppendage', semanticTraitId: null, maximumSubjectSize: 1, visible: false, baseWeight: 0.67 }),
  part({ id: 'tail_fish_fan', displayName: '扇形鱼尾', flavorText: '轻轻一摆，就把想象里的海水推开。', slotId: 'tail', description: 'a separate side-view rounded fan-shaped fish tail with soft translucent ribbing and a compact attachment base', rigId: 'floating', compatibleRigs: ['floating'], themeIds: ['deep-sea'], layer: 'rearAppendage', socket: 'tail', origin: { x: 512, y: 512 }, semanticTraitId: 'tail_fish_fan', maximumSubjectSize: 820, excludes: ['extra_moth_wings'] }),
  part({ id: 'tail_soft_curl', displayName: '卷卷软尾', flavorText: '尾巴末端总在画一个没有画完的圆。', slotId: 'tail', description: 'a separate thick curled plush tail shaped like a loose spiral, with a rounded attachment base and tip', rigId: 'blob', compatibleRigs: ['blob'], layer: 'rearAppendage', socket: 'tail', origin: { x: 512, y: 512 }, semanticTraitId: 'tail_soft_curl', maximumSubjectSize: 820 }),
  part({ id: 'tail_mushroom_cluster', displayName: '菌簇尾', flavorText: '尾巴上挤着三朵总想一起点头的小菌伞。', slotId: 'tail', description: 'a separate short plush tail ending in a cluster of three tiny rounded mushroom caps, compact and friendly', rigId: 'biped', compatibleRigs: ['biped'], themeIds: ['fungal'], layer: 'rearAppendage', socket: 'tail', origin: { x: 512, y: 512 }, semanticTraitId: 'tail_mushroom', maximumSubjectSize: 820 }),

  part({ id: 'extra_appendage_none', displayName: '没有额外附肢', flavorText: '两侧留白，刚好装下风。', slotId: 'extraAppendage', description: 'no visible extra appendage at all', rigId: 'blob', layer: 'rearAppendage', semanticTraitId: null, maximumSubjectSize: 1, visible: false, baseWeight: 0.08 }),
  part({ id: 'extra_moth_wings', displayName: '圆蛾翅', flavorText: '翅膀像两片会呼吸的软绒叶。', slotId: 'extraAppendage', description: 'a separate bilateral pair of small rounded moth wings with plush edges and simple soft eye-spot-free ribbing', rigId: 'biped', compatibleRigs: ['biped'], themeIds: ['fungal', 'shadow'], layer: 'rearAppendage', origin: { x: 512, y: 608 }, semanticTraitId: 'extra_moth_wings', maximumSubjectSize: 1500, excludes: ['tail_fish_fan'] }),
  ...
  part({ id: 'color_deep_sea_coral', displayName: '深海珊瑚配色', flavorText: '深蓝里藏着一小口温暖的珊瑚光。', slotId: 'colorScheme', description: 'a modular palette overlay of deep ocean-blue primary patches, aqua secondary patches, and warm-gold accents arranged in three broad body-following zones; color patches only', rigId: 'floating', themeIds: ['deep-sea'], layer: 'pattern', semanticTraitId: 'color_deep_sea', maximumSubjectSize: 1792 }),
  part({ id: 'color_fungal_amber', displayName: '菌沼琥珀配色', flavorText: '泥土色、奶油色和琥珀橙在一起发芽。', slotId: 'colorScheme', description: 'a modular palette overlay of olive primary patches, lime secondary patches, and warm-orange accents arranged in three broad body-following zones; color patches only', rigId: 'biped', themeIds: ['fungal'], layer: 'pattern', semanticTraitId: 'color_fungal', maximumSubjectSize: 1792 }),
  ...
  part({ id: 'effect_none', displayName: '没有特效', flavorText: '安安静静地站着，本身就足够奇妙。', slotId: 'effect', description: 'no visible effect at all', rigId: 'blob', layer: 'foregroundEffect', semanticTraitId: null, maximumSubjectSize: 1, visible: false, baseWeight: 0.15 }),
  part({ id: 'effect_bioluminescent_orbs', displayName: '浮游光泡', flavorText: '几颗小光泡像朋友一样围着它慢慢游。', slotId: 'effect', description: 'a modular foreground effect layer of seven small warm bioluminescent floating orbs with very soft halos, sparse and isolated', rigId: 'floating', themeIds: ['deep-sea', 'shadow'], layer: 'foregroundEffect', semanticTraitId: null, maximumSubjectSize: 1792, baseWeight: 0.35, boosts: { quirk_light_chaser: 1.3 } }),

...
import { sleepyCoatsApprovalPath, validateSleepyCoatsApproval } from './pixel-art-v3-sleepy-coats-approval.mjs'

const root = path.resolve(import.meta.dirname, '..')
const source = 'packages/asset-catalog/pixel/v3/sleepy-coats-1.5.0'
const release = 'packages/asset-catalog/pixel/v3/approved-1.5.0'
const dist = 'dist/pixel-art/v3-approved-1.5.0'
const reportPath = 'docs/qa/pixel-sleepy-coats/report.json'
...
const validated = await validateSleepyCoatsApproval(await read(sleepyCoatsApprovalPath), { candidate, candidateBytes, provenanceBytes, report, read })
const temp = path.join(root, 'node_modules/.cache/qmonster-pixel-sleepy-coats-approved')
...
const approvedContent = {
  ...
  evidence: { ...candidate.evidence, [sleepyCoatsApprovalPath]: validated.sha256 },
}
...
assert.equal(approved.coverage.length, 8064)
assert.equal(approved.generatable.length, 8064)
assert.equal(approved.coverage.filter(row => row.review === 'pending').length, 0)
assert.equal(Object.keys(approved.resources).length, 58)
const layers = {}
...
const provenance = {
  schemaVersion: 'pixel-art-sleepy-coats-approved-provenance-v1', status: 'approved', runtimeEnabled: false,
  candidate: validated.approval.candidate,
  approval: { path: sleepyCoatsApprovalPath, sha256: validated.sha256, statement: validated.approval.userStatement },
  approved: { artVersion: approved.artVersion, revision: approved.revision, sha256: sha(approvedBytes) },
  promotedCoverage: 2880,
  registration: { profiles: 28, coverage: 8064, approved: 8064, pending: 0, resources: 58 },
  files: { ...JSON.parse(provenanceBytes).files, ...validated.approval.evidence, [sleepyCoatsApprovalPath]: validated.sha256 },
}

...
- Metric fix: scales are frozen in one complete exact rig/head grammar table and are independent of tested connector width/depth declarations. Blob heads use `400×200`, floating heads use `300×180`; the frozen Task 6 biped mushroom guide is authoritatively `310×160`, while the other three newly approved Task 7 biped heads use `310×180`. There is no fallback for undeclared future heads. The identical baseline/scaled rejected biped mushroom scores `0.76875 / 0.21875 / 0.12997983870967741` in both cases, remaining above the unchanged `0.20 / 0.10 / 0.10` limits. Current 20-entry natural-neck classification remains passing.
...
### Final metric-evidence rereview correction

The final rereview found that the first immutable-scale implementation used a universal biped depth of `180`, which recomputed the approved natural-neck mushroom lobe as `0.1388888888888889` instead of its stored `0.15625`. Frozen Task 6 commit `f0292476de84c5c17b5fd8a392f8b00d0cdf3f7d`, its mushroom plug guide, current frozen-input integrity record, and approved manifest establish the mushroom grammar as `310×160` (`25/160 = 0.15625`). The other Task 7 biped heads are independently approved at `310×180` (`30/180`, `34/180`, and `31/180`), so a single universal biped depth is not authoritative.

The approval validator now rebuilds each exact body/head alpha composition in memory and recomputes all five stored metrics—`largestComponentRatio`, `centerlineGapPx`, `visibleTongueDepthRatio`, `visibleTongueAreaRatio`, and `centralLobeDepthRatio`—for every one of the 20 matrix entries directly from live exact-rig body/head sources and foreground/background masks, with tolerance `1e-12`. The renderer and validator share this read-only composition/metric implementation, so composition logic, algorithm, grammar, source, mask, or stored-summary drift blocks approval. Focused tests pass `5` files / `36` tests; approval CLI recomputes all `20` entries with zero diagnostics; typecheck passes; fresh single-worker full suite passes `73/73` files and `604` tests with `2` skipped. The nine approved artifacts, canonical acceptance, nine rejected artifacts, rejection record, 132 Task 6 inputs, and four Task 6 review files remain byte-identical.
